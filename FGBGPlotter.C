{
  ...
  /////////////DATA PARAMS/////////////
  const double MinEinc = 11.5;
  const double MaxEinc = 12.5;
  		
  const double MinEout =0.01;
  const double MaxEout = 2.5;
  
  const int NbinsPerDecade= 20;//THIS IS FOR THE OUTPUT!!!
  ...
  for (int l=1;l<=22;l++)
    {
      if (l==3) continue; //Skip ligl 3 cuz it sucks
      for (int p=1;p<=10;p++)
  	{
  	  sprintf(hfname,"dc1_3_diff_2eth_t00_ppac%02i_ligl%02i",p,l);//FG
...
// 	}
  ...
  tex8=new TLatex(0.6,0.35,mytex2);
  tex8->SetNDC();
  tex8->SetTextFont(43);
  tex8->SetTextSize(22);
  ...
  char mytex9[128];
  sprintf(mytex9,"<E> = %2.2f MeV",data->GetMean());
  tex9=new TLatex(0.6,0.25,mytex9);
  tex9->SetNDC();
  tex9->SetTextFont(43);
  tex9->SetTextSize(22);
