...
TH1D *var = new TH1D("tzeroRateVar_AllRuns","tzeroRateVar_AllRuns",10000000,0,100000);
TProfile *avvar = new TProfile("tzeroavvar","tzeroavvar",1,0,1);
...
Int_t VarRate_t0(){
  ...
  var->SetTitle("Kinda Variance in t_{0} Counting Rate");
  ...
  TFile *newef=TFile::Open("varhistos_t0.root","UPDATE");
  newef->cd();
  var->Write();
  avvar->Write();
  newef->Close();
...
void AnalyzeRun(string runstr){
  ...
  TH1F *tzero = (TH1F*)gDirectory->FindObjectAny("rate_s027_t00_hits");
  
  if (tzero)
    {  
      for (int i=1;i<=tzero->GetNbinsX();i++)
	{
	  if (removezeroes && tzero->GetBinContent(i)>100.)
	    {
	      temppro->Fill(0.5,tzero->GetBinContent(i));
	    }
	  if (!removezeroes) temppro->Fill(0.5,tzero->GetBinContent(i));
	}

      Double_t averagecounts = temppro->GetBinContent(1);
      Double_t countsthisrun = temppro->GetEntries();

      for (int j=1;j<=tzero->GetNbinsX();j++)
	{
	  if (removezeroes && tzero->GetBinContent(j)>100.)
	    {
	      var->Fill( (tzero->GetBinContent(j)-averagecounts)*(tzero->GetBinContent(j)-averagecounts) );
	      tempvar->Fill( (tzero->GetBinContent(j)-averagecounts)*(tzero->GetBinContent(j)-averagecounts)/countsthisrun );
	    }
	  if (!removezeroes) 
	    {
	      var->Fill( (tzero->GetBinContent(j)-averagecounts)*(tzero->GetBinContent(j)-averagecounts) );
