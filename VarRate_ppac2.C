...
TH1D *var = new TH1D("PpacRateVar_AllRuns","PpacRateVar_AllRuns",10000000,0,100000);
TProfile *avvar = new TProfile("Ppacavvar","Ppacavvar",1,0,1);
...
Int_t VarRate_ppac2(){
  ...
  var->SetTitle("Kinda Variance in Ppac-2 Counting Rate");
...
void AnalyzeRun(string runstr){
  ...
  TH1F *ppac2 = (TH1F*)gDirectory->FindObjectAny("rate_s032_ppac02_hits");
  
  if (ppac2)
    {  
      for (int i=1;i<=ppac2->GetNbinsX();i++)
	{
	  if (removezeroes && ppac2->GetBinContent(i)>5.)
	    {
	      temppro->Fill(0.5,ppac2->GetBinContent(i));
	    }
	  if (!removezeroes) temppro->Fill(0.5,ppac2->GetBinContent(i));
	}

      Double_t averagecounts = temppro->GetBinContent(1);
      Double_t countsthisrun = temppro->GetEntries();

      for (int j=1;j<=ppac2->GetNbinsX();j++)
	{
	  if (removezeroes && ppac2->GetBinContent(j)>5.)
	    {
	      var->Fill( (ppac2->GetBinContent(j)-averagecounts)*(ppac2->GetBinContent(j)-averagecounts) );
	      tempvar->Fill( (ppac2->GetBinContent(j)-averagecounts)*(ppac2->GetBinContent(j)-averagecounts)/countsthisrun );
	    }
	  if (!removezeroes) 
	    {
	      var->Fill( (ppac2->GetBinContent(j)-averagecounts)*(ppac2->GetBinContent(j)-averagecounts) );
  ...
  TFile *newef =TFile::Open("varhistos_ppac.root","UPDATE");
  newef->cd();
